...
%         eigval_strain(k,l)=max(abs(eig(strain)));
% 
%         [V,D] = eig(strain);
%         eig_vec = V(:,diag(D)==max(diag(D)));
%         eigvec_strain(k,l) = atan(eig_vec(2)/eig_vec(1));
%     end
% end
% 
% 
% % region_window = [0 1600 0 1600]; %[left,right,bottom,top]
% % region_window = [280 1320 280 1320]; %[left,right,bottom,top]
...
% eigvec_strain_cut = eigvec_strain(y_select',x_select);
% 
% x_plot = x(x_select);
...
% u_full = MatObj.pvx(:,:,FrameRange)*fps/ppcm;
% v_full = MatObj.pvy(:,:,FrameRange)*fps/ppcm;
% mean_v_cut = mean_v(y_select',x_select);
% mean_u_cut = mean_u(y_select',x_select);
% v_full_cut = v_full(y_select',x_select,:);
% v_full_cut = num2cell(v_full_cut,[1,2]);
% v_full_cut = vertcat(v_full_cut{:});
% 
...
% u2_interp = interp2(x0,y0,u2,px,py,'cubic');
% v2_interp = interp2(x0,y0,v2,px,py,'cubic');
% 
...
quiver(x_plot(Down_Samp)/L_scale,y_plot(Down_Samp)/L_scale,...
    u2_cut(Down_Samp,Down_Samp)/ppcm*fps*0.1,v2_cut(Down_Samp,Down_Samp)/ppcm*fps*0.15,...
    'off','-','LineWidth',0.5,'Color',[1,1,1]*0.3);

quiver(x_plot(Down_Samp_2)/L_scale,y_plot(Down_Samp_2)/L_scale,...
    cos(eigvec_strain_cut(Down_Samp_2,Down_Samp_2))*0.07,...
    sin(eigvec_strain_cut(Down_Samp_2,Down_Samp_2))*0.07,...
...
fillpatch_y = [mean(mean_v_cut)+std(v_full_cut),flip(mean(mean_v_cut)-std(v_full_cut))];
yyaxis left
